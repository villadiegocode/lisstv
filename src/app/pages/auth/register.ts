                        ...
                        <form [formGroup]="form" (ngSubmit)="submit()">
                            <div>
                                ...
                                <div class="flex flex-col gap-2 mb-8">
                                    ...
                                    @if (isValidField('email') || isSubmitted()) {
                                        @if (f['email'].errors?.['required']) {
                                            <p-message severity="error" variant="simple" size="small">{{ 'Email' | required: typeGenre.M }}</p-message>
                                        }

                                        @if (f['email'].errors?.['email']) {
                                            <p-message severity="error" variant="simple" size="small">{{ 'Email' | invalid }}</p-message>
                                        }
                                    }
                                </div>
                                ...
                                <div class="flex flex-col gap-2 mb-4">
                                    <p-password id="password1" formControlName="password" placeholder="Ingresar contraseña..." [toggleMask]="true" [fluid]="true" [feedback]="false"></p-password>
                                    @if (isValidField('password') || isSubmitted()) {
                                        @if (f['password'].errors?.['required']) {
                                            <p-message severity="error" variant="simple" size="small">{{ 'Contraseña' | required: typeGenre.F }}</p-message>
                                        }
                                    }
                                </div>
